...
Документация и примеры запуска SHALL ссылаться на образ с явным tag или digest (не плавающий неявный `latest` как единственный рекомендуемый pin). Версия запечённого модуля SHALL соответствовать релизу/тегу образа.

#### Scenario: README pins image reference
- **WHEN** оператор следует happy path в README для `docker run`
- **THEN** пример использует image reference с явным tag или digest
